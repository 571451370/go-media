...
*/
static int
TTF_underline_top_row(TTF_Font * font)
{
...
*/
static int
TTF_Glyph_underline_top_row(TTF_Font * font, c_glyph * glyph)
{
...
*/
static int
TTF_underline_bottom_row(TTF_Font * font)
{
...
*/
static int
TTF_Glyph_underline_bottom_row(TTF_Font * font, c_glyph * glyph)
{
...
*/
static int
TTF_strikethrough_top_row(TTF_Font * font)
{
...
*/
static int
TTF_Glyph_strikethrough_top_row(TTF_Font * font, c_glyph * glyph)
{
	return TTF_strikethrough_top_row(font) - font->ascent + glyph->maxy;
}

static void
TTF_initLineMectrics(const TTF_Font * font, const SDL_Surface * textbuf,
    const int row, Uint8 ** pdst, int *pheight)
{
	Uint8 *dst;
	int height;
...
*/
static void
TTF_drawLine_Solid(const TTF_Font * font, const SDL_Surface * textbuf,
    const int row)
{
	int line;
	Uint8 *dst_check =
	    (Uint8 *) textbuf->pixels + textbuf->pitch * textbuf->h;
	Uint8 *dst;
	int height;
...
*/
static void
TTF_drawLine_Shaded(const TTF_Font * font, const SDL_Surface * textbuf,
    const int row)
{
	int line;
	Uint8 *dst_check =
...
*/
static void
TTF_drawLine_Blended(const TTF_Font * font, const SDL_Surface * textbuf,
    const int row, const Uint32 color)
{
	int line;
	Uint32 *dst_check =
...
Load_Glyph(TTF_Font * font, Uint16 ch, c_glyph * cached, int want)
{
	...
	error =
	    FT_Load_Glyph(face, cached->index,
	    FT_LOAD_DEFAULT | font->hinting);
	if (error) {
		return error;
	}
	...
	if ((want & CACHED_METRICS) && !(cached->stored & CACHED_METRICS)) {
		if (FT_IS_SCALABLE(face)) {
			...
			cached->maxx =
			    FT_CEIL(metrics->horiBearingX + metrics->width);
			...
			cached->miny =
			    cached->maxy -
			    FT_CEIL(face->available_sizes[font->
				font_size_family].height);
			cached->yoffset = 0;
			cached->advance = FT_CEIL(metrics->horiAdvance);
		}
	...
	if (((want & CACHED_BITMAP) && !(cached->stored & CACHED_BITMAP)) ||
		...
		if ((font->outline > 0)
			...
			FT_Stroker_Set(stroker, font->outline * 64,
			...
			/* Render the glyph */
			error =
			    FT_Glyph_To_Bitmap(&bitmap_glyph,
		...
		if (dst->rows != 0) {
			dst->buffer =
			    (unsigned char *) SDL_malloc(dst->pitch *
			    dst->rows);
			if (!dst->buffer) {
		...
		if (TTF_HANDLE_STYLE_BOLD(font)) {
			...
			for (row = dst->rows - 1; row >= 0; --row) {
				pixmap =
				    (Uint8 *) dst->buffer + row * dst->pitch;
				for (offset = 1;
				    offset <= font->glyph_overhang; ++offset) {
...
static void
LATIN1_to_UTF8(const char *src, Uint8 * dst)
{
	while (*src) {
		Uint8 ch = *(Uint8 *) src++;
		if (ch <= 0x7F) {
...
static void
UCS2_to_UTF8(const Uint16 * src, Uint8 * dst)
{
...
static Uint32
UTF8_getch(const char **src, size_t * srclen)
{
...
int
TTF_GlyphIsProvided(const TTF_Font * font, Uint16 ch)
{
    ...
    const char *text, SDL_Color fg, SDL_Color bg)
{
	...
	for (index = 0; index < NUM_GRAYS; ++index) {
		palette->colors[index].r =
		    bg.r + (index * rdiff) / (NUM_GRAYS - 1);
		palette->colors[index].g =
		    bg.g + (index * gdiff) / (NUM_GRAYS - 1);
		palette->colors[index].b =
		    bg.b + (index * bdiff) / (NUM_GRAYS - 1);
	}
...
SDL_Surface *
TTF_RenderText_Blended_Wrapped(TTF_Font * font, const char *text,
    SDL_Color fg, Uint32 wrapLength)
{
	...
	if (utf8) {
		LATIN1_to_UTF8(text, utf8);
		surface =
		    TTF_RenderUTF8_Blended_Wrapped(font, (char *) utf8, fg,
		    wrapLength);
		SDL_stack_free(utf8);
	} else {
		SDL_OutOfMemory();
...
static SDL_bool
CharacterIsDelimiter(char c, const char *delimiters)
{
	while (*delimiters) {
		if (c == *delimiters) {
			return SDL_TRUE;
...
SDL_Surface *
TTF_RenderUTF8_Blended_Wrapped(TTF_Font * font,
    const char *text, SDL_Color fg, Uint32 wrapLength)
{
	...
	if (wrapLength > 0 && *text) {
		...
		do {
			strLines =
			    (char **) SDL_realloc(strLines,
			    (numLines + 1) * sizeof(*strLines));
			if (!strLines) {
				TTF_SetError("Out of memory");
				return (NULL);
			}
	...
	for (line = 0; line < numLines; line++) {
		...
		while (textlen > 0) {
			...
			if (use_kerning && prev_index && glyph->index) {
				FT_Vector delta;
				FT_Get_Kerning(font->face, prev_index,
			...
			for (row = 0; row < glyph->pixmap.rows; ++row) {
				...
				dst =
				    ((Uint32 *) textbuf->pixels +
				    rowSize * line) + (row +
				    glyph->yoffset) * textbuf->pitch / 4 +
				    xstart + glyph->minx;
				...
				src =
				    (Uint8 *) (glyph->pixmap.buffer +
				    glyph->pixmap.pitch * row);
				for (col = width; col > 0 && dst < dst_check;
				    --col) {
					alpha = *src++;
					*dst++ |= pixel | (alpha << 24);
				}
...
SDL_Surface *
TTF_RenderUNICODE_Blended_Wrapped(TTF_Font * font, const Uint16 * text,
    SDL_Color fg, Uint32 wrapLength)
{
	SDL_Surface *surface = NULL;
	Uint8 *utf8;
...
{
	FT_Vector delta;
	FT_Get_Kerning(font->face, prev_index, index, ft_kerning_default,
	    &delta);
	return (delta.x >> 6);
}
...
TTF_GetFontKerningSizeGlyphs(TTF_Font * font, Uint16 previous_ch, Uint16 ch)
{
	...
	if (previous_ch == UNICODE_BOM_NATIVE
	    || previous_ch == UNICODE_BOM_SWAPPED) {
		return 0;
	}
	...
	error =
	    FT_Get_Kerning(font->face, prev_index, glyph_index,
	    ft_kerning_default, &delta);
	if (error) {
...
int
TTF_SizeUTF8Ex(TTF_Font * font, const char *text, size_t textlen, int *w,
    int *h)
{
	int status;
	int x, z;
